...
case "$1" in
    ...
    "health")
        ...
        while [ $elapsed -lt $timeout ]; do
            mysql_status=$(docker inspect --format='{{.State.Health.Status}}' aicmr-mysql-dev 2>/dev/null || echo "unknown")
            redis_status=$(docker inspect --format='{{.State.Health.Status}}' aicmr-redis-dev 2>/dev/null || echo "unknown")
            backend_status=$(docker ps --filter "name=aicmr-backend-dev" --format '{{.Status}}' 2>/dev/null || echo "")
            frontend_status=$(docker ps --filter "name=aicmr-frontend-dev" --format '{{.Status}}' 2>/dev/null || echo "")
